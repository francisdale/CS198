...
public class FaceDetectTask extends AsyncTask<Void, Void, Void> {
    ...
    private static final int ATTENDANCE_USAGE = 0;
    private static final int TRAIN_USAGE = 1;
    ...
    protected Void doInBackground(Void[] params) {
        try {
            ...
            GenQueue<String> detectQueue = td.detectQueue;
            ...
            if (usageType == ATTENDANCE_USAGE){
                ...
                while(td.isUIOpened()){
                    while(detectQueue.hasItems()){
                        mGray = imread(detectQueue.poll(), CV_LOAD_IMAGE_GRAYSCALE);
                        ...
                        //Crop faces both in color and grayscale:
                        faceCount = faces.capacity();
                        for (int i = 0; i < faceCount; i++) {
                            opencv_core.Rect r = faces.position(i);
                            x = r.x();
                            y = r.y();

                            roi = new Rect(x, y, r.width(), r.height());
                            recogQueue.add(new Mat(mGray, roi));
                        }
                    }
                }
                ...
                while(td.isUIOpened()){
                    while(detectQueue.hasItems()){
                        mColor = imread(detectQueue.poll());
